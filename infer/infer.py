...
_FALLBACK_MODEL_CFG = {
    ...
    'user_ns_tokens': 0,
    'item_ns_tokens': 0,
    'use_dense_guided_user_pooling': True,
    'dense_guided_user_fids': '62,63,64,65,66,89,90,91',
    'exclude_aligned_dense_feats': True,
    'use_item_oof_gate': True,
    'item_oof_gate_hidden_dim': 16,
}
...
def build_feature_specs(
    ...
    for _fid, offset, length in schema.entries:
...
def build_model(
    ...
    aligned_fids = [
        int(fid.strip()) for fid in str(model_cfg.get('dense_guided_user_fids', '')).split(',')
        if fid.strip()
    ]
    ...
    ctor_cfg = dict(model_cfg)
    ctor_cfg.pop('dense_guided_user_fids', None)

    logging.info(f"Building PCVRHyFormer with cfg: {ctor_cfg}")
    model = PCVRHyFormer(
        user_int_feature_specs=user_int_feature_specs,
        item_int_feature_specs=item_int_feature_specs,
        user_dense_dim=dataset.user_dense_schema.total_dim,
        item_dense_dim=dataset.item_dense_schema.total_dim,
        item_oof_dense_dim=dataset.item_oof_dense_schema.total_dim,
...
def _batch_to_model_input(
    ...
    for domain in seq_domains:
        ...
        seq_time_cal[domain] = device_batch.get(
                f'{domain}_time_cal',
                torch.zeros(B, L, 3, dtype=torch.long, device=device))
...
def main() -> None:
    ...
    item_oof_feature_table_path = None
    item_oof_artifact_cfg = train_config.get('item_oof_artifact_path')
    if item_oof_artifact_cfg:
        local_candidate = os.path.join(model_dir, os.path.basename(item_oof_artifact_cfg))
        if os.path.exists(local_candidate):
            item_oof_feature_table_path = local_candidate
        elif os.path.exists(item_oof_artifact_cfg):
            item_oof_feature_table_path = item_oof_artifact_cfg
    ...
    test_dataset = PCVRParquetDataset(
        ...
        buffer_batches=0,
        is_training=False,
        item_oof_feature_table_path=item_oof_feature_table_path,
        item_oof_smoothing_alpha=float(train_config.get('item_oof_smoothing_alpha', 100.0)),
    )
