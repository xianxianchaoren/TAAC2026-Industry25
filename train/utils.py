...
class EMA:
    """Maintain an exponential moving average over model parameters.

    This project only uses the dense-only variant during training so that
    giant sparse Embedding tables are excluded from the shadow copy.
    """
    ...
    def __init__(
        ...
        tracked_ptrs: Optional[Set[int]] = None
        if dense_only and hasattr(model, 'get_dense_params'):
            tracked_ptrs = {p.data_ptr() for p in model.get_dense_params()}

        self.shadow: Dict[str, torch.Tensor] = {}
        for name, param in model.named_parameters():
            if not param.requires_grad:
                continue
            if tracked_ptrs is not None and param.data_ptr() not in tracked_ptrs:
                continue
            self.shadow[name] = param.detach().clone()

        logging.info(
            "EMA enabled: tracked_params=%s, decay=%s, warmup_steps=%s, dense_only=%s",
            len(self.shadow), self.decay, self.warmup_steps, self.dense_only,
        )
    ...
    def apply_to(self, model: nn.Module) -> None:
        if self.backup:
            raise RuntimeError("EMA.apply_to() called before restore()")
