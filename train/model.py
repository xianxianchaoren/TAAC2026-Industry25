...
from aligned_pooling import AlignedUserDenseGuidedToken
...
class ModelInput(NamedTuple):
    ...
    item_oof_dense_feats: torch.Tensor
    seq_data: dict        # {domain: tensor [B, S, L]}
    seq_lens: dict        # {domain: tensor [B]}
    seq_time_buckets: dict  # {domain: tensor [B, L]}
    seq_time_cal: dict    # {domain: tensor [B, L, 3]}  hour/dayofweek/month
...
class FFN(nn.Module):
    ...
    def __init__(self, d1: int, d2: int, dropout: float):
        super().__init__()
        self.fc1 = nn.Linear(d1, d2, bias=False)
        self.fc2 = nn.Linear(d1, d2, bias=False)
        self.fc3 = nn.Linear(d2, d1, bias=False)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.fc3(self.dropout(F.silu(self.fc1(x)) * self.fc2(x)))
...
class LongerEncoder(nn.Module):
    ...
    def _gather_top_k(
        ...
        # Start position for each sample: 0 (since it's reverse chronological)
        actual_k = torch.clamp(valid_len, max=self.top_k)  # (B,)
        start_pos = torch.zeros_like(valid_len)  # (B,)
        ...
        # New padding mask: positions >= actual_k are padding (right padding)
        pos_indices = torch.arange(self.top_k, device=device).unsqueeze(0)  # (1, top_k)
        new_padding_mask = pos_indices >= actual_k.unsqueeze(1)  # (B, top_k)
    ...
    def forward(
        ...
        if L > self.top_k:
            ...
            # Causal mask (Reverse chronological)
            attn_mask = None
            if self.causal:
                # Since sequence is reverse chronological (newest first), 
                # token at index i (time t) should only attend to indices [i, ..., L-1] (older events).
                # The attention mask expects True for masked positions.
                # We need an upper triangular mask of False (allowed) and lower triangular of True (masked).
                # Example for L=3:
                # [False, False, False] (token 0 can see 0,1,2)
                # [True, False, False]  (token 1 can see 1,2)
                # [True, True, False]   (token 2 can see 2)
                attn_mask = torch.tril(torch.ones(L, L, dtype=torch.bool, device=x.device), diagonal=-1)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        use_dense_guided_user_pooling: bool = False,
        aligned_user_pair_specs: Optional[List[Dict[str, int]]] = None,
        exclude_aligned_dense_feats: bool = False,
        use_item_oof_gate: bool = True,
        item_oof_gate_hidden_dim: int = 16,
    ) -> None:
        super().__init__()
        ...
        self.use_dense_guided_user_pooling = use_dense_guided_user_pooling
        self.aligned_user_pair_specs = aligned_user_pair_specs or []
        self.exclude_aligned_dense_feats = exclude_aligned_dense_feats
        self.use_item_oof_gate = use_item_oof_gate
        self.item_oof_gate_hidden_dim = item_oof_gate_hidden_dim
        # Number of extra time features cat-ted into emb_list per token position.
        # timebucket (if enabled) + hour + dow + month
        self._num_time_feats = (1 if num_time_buckets > 0 else 0) + 3
        ...
        if self.has_user_dense:
            ...
            if self.use_dense_guided_user_pooling and self.aligned_user_pair_specs:
                self.user_aligned_dense_token = AlignedUserDenseGuidedToken(
                    pair_specs=self.aligned_user_pair_specs,
                    emb_dim=emb_dim,
                    d_model=d_model,
                )
                
                if self.exclude_aligned_dense_feats:
                    # Compute mask for remaining dense features
                    remaining_indices = set(range(user_dense_dim))
                    for spec in self.aligned_user_pair_specs:
                        start = spec['dense_offset']
                        end = start + spec['dense_length']
                        for i in range(start, end):
                            remaining_indices.discard(i)
                    
                    self.remaining_dense_indices = sorted(list(remaining_indices))
                    remaining_dim = len(self.remaining_dense_indices)
                    
                    self.user_dense_proj = nn.Sequential(
                        nn.Linear(remaining_dim, d_model),
                        nn.LayerNorm(d_model),
                    )
                else:
                    self.remaining_dense_indices = None
                    self.user_dense_proj = nn.Sequential(
                        nn.Linear(user_dense_dim, d_model),
                        nn.LayerNorm(d_model),
                    )

                self.user_dense_fuse = nn.Sequential(
        ...
        self.has_item_oof_dense = item_oof_dense_dim > 0
        if self.has_item_oof_dense:
            self.item_oof_dense_proj = nn.Sequential(
                nn.Linear(item_oof_dense_dim, d_model),
                nn.LayerNorm(d_model),
            )
            if self.use_item_oof_gate:
                self.item_oof_gate = nn.Sequential(
                    nn.Linear(3, item_oof_gate_hidden_dim),
                    nn.SiLU(),
                    nn.Linear(item_oof_gate_hidden_dim, 1),
                )

        # Total NS token count
        self.num_ns = (num_user_ns + (1 if self.has_user_dense else 0)
                       + num_item_ns + (1 if self.has_item_dense else 0)
                       + (1 if self.has_item_oof_dense else 0))
        ...
        # Absolute Position Embedding (if configured)
        if self.use_abs_pos:
            self.abs_pos_emb = nn.Embedding(max_abs_pos, d_model)

        # Domain ID Embedding (if configured)
        if self.use_domain_emb:
            self.domain_emb = nn.Embedding(self.num_sequences, d_model)
            # Map domain name to its integer index for lookup
            self.domain_to_idx = {domain: i for i, domain in enumerate(self.seq_domains)}
        ...
        # Uses emb_dim (not d_model) so it participates in the emb_list cat path.
        if num_time_buckets > 0:
            self.time_embedding = nn.Embedding(num_time_buckets, d_model, padding_idx=0)
        
        # ================== Candidate-Aware Seq Pre-Attention ==================
        self.item_int_candidate_pre_attns = nn.ModuleList([
            CrossAttention(
                d_model=d_model,
                num_heads=num_heads,
        ...
        # ================== Calendar Time Embeddings (hour/dayofweek/month) ==================
        # hour: 0=padding, 1-24; dayofweek: 0=padding, 1-7; month: 0=padding, 1-12
        # Also emb_dim so they slot into the emb_list cat path uniformly.
        self.hour_embedding = nn.Embedding(25, emb_dim, padding_idx=0)
        self.dow_embedding = nn.Embedding(8, emb_dim, padding_idx=0)
        self.month_embedding = nn.Embedding(13, emb_dim, padding_idx=0)
    ...
    def _init_params(self) -> None:
        ...
        for emb in [self.hour_embedding, self.dow_embedding, self.month_embedding]:
            nn.init.xavier_normal_(emb.weight.data)
            emb.weight.data[0, :] = 0
    ...
    def _embed_seq_domain(
        ...
        time_cal_ids: Optional[torch.Tensor] = None,
        ...
        for i in range(S):
            ...
            if real_idx == -1:
                ...
                emb_list.append(e)
        # Append time embeddings into emb_list so they are cat-ted and projected together.
        # timebucket: (B, L, emb_dim); cal: hour/dow/month each (B, L, emb_dim).

        # if time_cal_ids is not None:
        #     emb_list.append(self.hour_embedding(time_cal_ids[..., 0]))
        #     emb_list.append(self.dow_embedding(time_cal_ids[..., 1]))
        #     emb_list.append(self.month_embedding(time_cal_ids[..., 2]))

        cat_emb = torch.cat(emb_list, dim=-1)  # (B, L, (S + num_time_feats)*emb_dim)
        token_emb = F.gelu(proj(cat_emb))  # (B, L, D)

        if self.num_time_buckets > 0:
            token_emb = token_emb + self.time_embedding(time_bucket_ids)  # (B, L, emb_dim)

        # Add absolute positional encoding (if configured)
        if self.use_abs_pos:
            device = seq.device
            # Generate positions: 0, 1, ..., L-1
            # Note: For reverse chronological, position 0 corresponds to the most recent event
            positions = torch.arange(L, dtype=torch.long, device=device)  # (L,)
            # Clamp to prevent out-of-bounds if L > max_abs_pos
            positions = torch.clamp(positions, max=self.abs_pos_emb.num_embeddings - 1)
            pos_emb = self.abs_pos_emb(positions)  # (L, D)
            
            # Mask out padding positions for absolute position embedding
            if padding_mask is not None:
                valid_mask = (~padding_mask).unsqueeze(-1).float()  # (B, L, 1)
                pos_emb_masked = pos_emb.unsqueeze(0) * valid_mask  # (B, L, D)
                token_emb = token_emb + pos_emb_masked
            else:
                token_emb = token_emb + pos_emb.unsqueeze(0)  # (B, L, D)

        # Add domain ID embedding (if configured)
        if self.use_domain_emb and domain_name is not None:
            domain_idx = self.domain_to_idx[domain_name]
            domain_idx_tensor = torch.tensor(domain_idx, dtype=torch.long, device=seq.device)
            d_emb = self.domain_emb(domain_idx_tensor)  # (D,)
            
            # Mask out padding positions for domain embedding
            if padding_mask is not None:
                valid_mask = (~padding_mask).unsqueeze(-1).float()  # (B, L, 1)
                d_emb_masked = d_emb.view(1, 1, self.d_model) * valid_mask  # (B, L, D)
                token_emb = token_emb + d_emb_masked
            else:
                token_emb = token_emb + d_emb.view(1, 1, self.d_model)  # (B, L, D)
    ...
    def _build_user_dense_token(self, user_int_feats: torch.Tensor, user_dense_feats: torch.Tensor) -> torch.Tensor:
        """Build the single user-dense token, optionally fusing aligned pooling."""
        if not (self.use_dense_guided_user_pooling and self.aligned_user_pair_specs):
            return F.silu(self.user_dense_proj(user_dense_feats))

        aligned_tok = self.user_aligned_dense_token(user_int_feats, user_dense_feats)
        
        # Extract remaining dense features using the indices if exclude_aligned_dense_feats is True
        if getattr(self, 'exclude_aligned_dense_feats', False) and getattr(self, 'remaining_dense_indices', None) is not None:
            remaining_feats = user_dense_feats[:, self.remaining_dense_indices]
            raw_user_dense_tok = F.silu(self.user_dense_proj(remaining_feats))
        else:
            raw_user_dense_tok = F.silu(self.user_dense_proj(user_dense_feats))
            
        fused = torch.cat([raw_user_dense_tok, aligned_tok], dim=-1)
        return F.silu(self.user_dense_fuse(fused))

    def _build_item_oof_token(
        self,
        item_oof_dense_feats: torch.Tensor,
        item_dense_tok: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """Project item OOF features and gate them toward stable item dense features."""
        item_oof_tok = F.silu(self.item_oof_dense_proj(item_oof_dense_feats))
        if not (self.use_item_oof_gate and item_dense_tok is not None):
            return item_oof_tok

        B = item_oof_dense_feats.shape[0]
        gate_inputs = item_oof_dense_feats.new_zeros(B, 3)
        if item_oof_dense_feats.shape[1] > 3:
            gate_inputs[:, 0] = item_oof_dense_feats[:, 3]  # show_cnt_24h_log1p
        if item_oof_dense_feats.shape[1] >= 2:
            gate_inputs[:, 1:] = item_oof_dense_feats[:, -2:]  # is_matched, confidence
        gate = torch.sigmoid(self.item_oof_gate(gate_inputs))
        return gate * item_oof_tok + (1.0 - gate) * item_dense_tok

    def _apply_item_int_candidate_pre_attn(
        self,
        seq_tokens_list: List[torch.Tensor],
        seq_masks_list: List[torch.Tensor],
        item_ns: torch.Tensor,
    ) -> List[torch.Tensor]:
        """Condition each domain's history tokens on current candidate item tokens."""
        # if not self.use_item_int_candidate_pre_attn:
        #     return seq_tokens_list

        conditioned = []
        for i, (seq_tokens, seq_mask) in enumerate(zip(seq_tokens_list, seq_masks_list)):
            updated = self.item_int_candidate_pre_attns[i](
                query=seq_tokens,
                key_value=item_ns,
                key_padding_mask=None,
            )
            updated = updated.masked_fill(seq_mask.unsqueeze(-1), 0.0)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        if self.has_user_dense:
            user_dense_tok = self._build_user_dense_token(
                inputs.user_int_feats, inputs.user_dense_feats).unsqueeze(1)  # (B, 1, D)
            ns_parts.append(user_dense_tok)
        ns_parts.append(item_ns)
        item_dense_tok = None
        if self.has_item_dense:
            item_dense_tok = F.silu(self.item_dense_proj(inputs.item_dense_feats))
            ns_parts.append(item_dense_tok.unsqueeze(1))
        if self.has_item_oof_dense:
            item_oof_dense_tok = self._build_item_oof_token(
                inputs.item_oof_dense_feats, item_dense_tok=item_dense_tok)
            ns_parts.append(item_oof_dense_tok.unsqueeze(1))
        ...
        for domain in self.seq_domains:
            ...
            tokens = self._embed_seq_domain(
                ...
                time_cal_ids=inputs.seq_time_cal.get(domain) if inputs.seq_time_cal else None,
                domain_name=domain,
                padding_mask=mask)
            seq_tokens_list.append(tokens)

        seq_tokens_list = self._apply_item_int_candidate_pre_attn(
            seq_tokens_list, seq_masks_list, item_ns)
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.has_user_dense:
            user_dense_tok = self._build_user_dense_token(
                inputs.user_int_feats, inputs.user_dense_feats).unsqueeze(1)
            ns_parts.append(user_dense_tok)
        ns_parts.append(item_ns)
        item_dense_tok = None
        if self.has_item_dense:
